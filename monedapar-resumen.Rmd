...
```{r objetos-python}
avales = read_csv("output/final/avales_history_clean.csv", col_type=cols())
txs = read_csv("output/final/txs_history_clean.csv", col_type=cols())

nodos = list.files(path="output/final/resumen/m/") %>% str_match('\\_([:alpha:]+)\\.csv') %>%
  "["(,2) %>% unique
